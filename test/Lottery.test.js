...
describe("Lottery contract", () => {
  ...
  it("assigns manager address", async () => {
    let results = await lottery.methods.manager().call();
    assert.equal(accounts[0], results);
    // console.log(results);
  });

  it("allows one to enter the lottery", async () => {
  ...
  it("allows multiple to enter the lottery", async () => {
    await lottery.methods.enter().send({
      from: accounts[1],
      value: web3.utils.toWei("0.02", "ether"),
    });
  ...
  it("does not allow the manager to participate", async () => {
    try {
      await lottery.methods.enter().send({
        from: accounts[0],
        value: web3.utils.toWei("0.02", "ether"),
  ...
  it("sends money and resets the players array", async () => {
